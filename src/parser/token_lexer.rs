...
impl<'s> Lexer<'s> {
    ...
    fn step(&mut self) {
        ...
        if ch == b'"' {
            self.lex_string(tok_line, tok_col, tok_indent);
            return;
        }
        if ch == b'\'' {
            self.lex_char(tok_line, tok_col, tok_indent);
            return;
        }
        if ch.is_ascii_digit() {
            self.lex_number(tok_line, tok_col, tok_indent);
            return;
        }
    ...
    // Reconhece todos os operadores e pontuações do Pawn (incluindo os de 2-4
    // caracteres) via longest-match. O tamanho reflete a quantidade de símbolos
    // da linguagem, não complexidade entrelaçada.
...
mod tests {
    ...
    fn string_with_color_codes_no_brace_tokens() {
        ...
        assert!(
            !ts.tokens
                .iter()
                .any(|t| matches!(t.kind, TokenKind::LBrace | TokenKind::RBrace))
        );
    }
    ...
    fn tab_then_spaces_indent() {
        ...
        let src_tab_spaces = "\t    foo(x);";
        let src_spaces = "        foo(x);";
        ...
        assert_eq!(
            indent1, indent2,
            "\\t+4 espaços deve = 8 espaços com tabsize=4"
        );
    }
    ...
    fn multiline_string_continuation_indent() {
        ...
        let comma_after_str = ts
            ...
            .unwrap();
        assert_eq!(
            comma_after_str.stmt_indent, 8,
            "vírgula após string multilinha deve ter indent=8"
        );
        // return deve ter indent=4
        let ret = ts
            .tokens
            .iter()
            .find(|t| t.kind == TokenKind::Ident && t.value == "return")
            .unwrap();

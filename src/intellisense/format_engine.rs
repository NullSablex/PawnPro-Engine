...
fn analyze_lines(text: &str) -> Vec<LineInfo> {
    ...
    for (i, raw) in phys.iter().enumerate() {
        ...
        // Continuação dentro de parênteses abertos: +1. Exceto se a linha COMEÇA
        // fechando o grupo (ex.: o `)` que encerra a lista de parâmetros), que
        // alinha com a linha de abertura.
        let starts_closing = trimmed_raw.starts_with(')') || trimmed_raw.starts_with(']');
        let continuation = u32::from(paren_balance > 0 && !starts_closing);
